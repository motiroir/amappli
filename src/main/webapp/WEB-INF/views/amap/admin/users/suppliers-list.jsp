				...
				<div class="col-12">
					...
					<div class="table-container d-flex justify-content-between align-items-center">
						...
						<a href="<c:url value='/amap/contracts/form' />" class="btn-create">
							<span class="icon">+</span>Ajouter un fournisseur
						</a>
					</div>
					<!-- Mode tableau -->
					<table class="table">
						...
						<tbody>
							<c:forEach var="producer" items="${producers}">
								<tr>
									<td>${producer.domainName}</td>
									<td>${producer.producerName}</td>
									<td>
										<div class='d-flex justify-content-start align-items-center'>
											<a href="<c:url value='tenancies/{tenancyId}/amap/admin/backoffice/${producer.id}' />"
												class="btn-view"> <i class="bi bi-eye"></i>
											</a>
											<form method="POST"
												action="<c:url value='/amap/producers/delete/${producer.id}' />"
												style="display: inline;">
												<button type="submit" class="btn-delete"
													onclick="return confirm('Voulez-vous vraiment supprimer le fournisseur ${producer.producerName} ?');">
