document.addEventListener("DOMContentLoaded", () => {
	const productsContainer = document.querySelector(".row-cols-1"); // Conteneur des produits
	const sortBySelect = document.getElementById("sortByProducts");
	const searchBar = document.getElementById("searchBar");

	let products = Array.from(document.querySelectorAll(".row-cols-1 .col")); // Liste des produits
	...
	function sortProducts(criteria) {
		products.sort((a, b) => {
			...
			//            const dateA = new Date(a.querySelector(".card-text.text-muted").innerText.replace("DLC:", "").trim());
			//            const dateB = new Date(b.querySelector(".card-text.text-muted").innerText.replace("DLC:", "").trim());
